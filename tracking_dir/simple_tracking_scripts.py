...
def get_new_loc(ds, x, y, hw, t, level):
...
def track_init(ds, stat_Q, CS_tracks_list, our_time):
    ...
    for i in range(len(stat_Q)):
        ...
        ts.append(our_time)
        times.append(ds.XTIME[our_time].values)
...
def simple_advection_tracking(ds, our_time, circ='C'):
    CS_tracks_list = []
    ...
    CS_tracks_list = track_init(ds, stat_Q, CS_tracks_list, our_time)
    ...
    for t in tqdm(range(our_time+1,len(ds.Time))):
        ...
        for i in range(len(CS_tracks_list)):
            ...
            if ~np.isnan(x_init):
                ...
                x, y = get_new_loc(ds, x_init, y_init, int(np.round(rad_init)), t, -3)
...
#                     print('stat', stat_Q_c)
                    ...
                    CS_tracks_list[i]['time'].append(ds.XTIME[t].values)
...
#                     CS_tracks_list.remove(CS)
        ...
        if len(stat_Q) != 0:
            ...
            CS_tracks_list = track_init(ds, stat_Q.reset_index(), CS_tracks_list, t)
