...
def simple_advection_tracking(ds, data_type, our_time, circ='C'):
    ...
    CS_tracks_list = track_init(ds, data_type, stat_Q, CS_tracks_list, our_time, time_unit)
    
    dt_step = np.timedelta64(ds[time_unit].values[1] - ds[time_unit].values[0], 's').astype(int)   
    
    for t in tqdm(range(our_time+1,len(ds[time_unit]))):
        
        coords_Q = vortex.for_DBSCAN(ds[crit][t, our_level], th, crit)  
        coords_Q, n_clusters_lambda2 = clustering_DBSCAN_rortex_C(coords_Q, min_samples=min_samples, eps=eps, circ=circ)
        coords_Q, n_clusters_Q = vortex.DBSCAN_filter(coords_Q, points=CS_points_th)
        ...
        for i in range(len(CS_tracks_list)):
            ...
            if ~np.isnan(x_init) and x_init != (len(ds[x_unit]) - 1) and y_init != (len(ds[y_unit]) - 1) and x_init != 0 and y_init != 0:
                ...
                if len(CS_tracks_list[i]['x']) == 1:
                    x, y = get_new_loc_mean_speed(ds, dt_step, x_init, y_init, int(np.round(rad_init)), t, 0)
                else:
...
#             print(f'new init for {len(stat_Q)} CSs')
            CS_tracks_list = track_init(ds, data_type, stat_Q.reset_index(), CS_tracks_list, t, time_unit)
