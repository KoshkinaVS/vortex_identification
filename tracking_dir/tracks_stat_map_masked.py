...
variables = {
    'density': {'var_name': 'density', 'vmax':  1.5, 'vstep':1, 'cmap':cmaps.WhiteBlueGreenYellowRed},
    ...
    'LH': {'var_name': 'LH', 'vmax':  230, 'vstep':1, 'cmap':cmaps.BlueDarkRed18},
    'HFX': {'var_name': 'HFX', 'vmax':  150, 'vstep':1, 'cmap':cmaps.BlueDarkRed18},
...
path_dir_raw = f'/storage/NAD/NAAD/LoRes/2018'

param_ds = 5
name = f'wrfout_d01_2018*'
ls = list(sorted(Path(f"{path_dir_raw}").glob(f'{name}')))
ds = xr.open_dataset(ls[0])['LANDMASK'][0,param_ds:-param_ds,param_ds:-param_ds]
yx_land = np.argwhere(ds.values == 1)
        ...
        if os.path.exists(path_data_new):
            ...
            if len(ls) != 0:
                for ii,ifile in enumerate(ls):
                    ...
                    df = df.drop(df.columns[0], axis=1)
                    df_coords = np.array([df['y'].values, df['x'].values]).T
                    if ~(df_coords[:,None] == yx_land).all(2).any(1).all():
...
def compute_density_square_multi(CS_dict, season, params, param_sq=3):    
    ...
    for param in params:
        ...
        np.save(f'{path_data_dir_density}/masked_{tracking_type}_{circ}_track_density_{season}_multi_{param}.npy', map_dict_multi[param])
...
def plot_density(track_density, name, season, param_sq, vmax, cmap=cmaps.WhiteBlueGreenYellowRed, vmin=0, save=False):    
    ...
    if save:
        if not os.path.exists(folder):
            os.makedirs(folder)
        plt.savefig(f"{folder}/masked_{circ}_{name}_{season}_sq_{param_sq}.png", dpi=150, bbox_inches="tight", transparent=True)
...
if precomputed:
    ...
    for param in params:
        ...
        map_dict_winter = np.load(f'{path_data_dir_density}/masked_{tracking_type}_{circ}_track_density_winter_multi_{param}.npy')
        map_dict_summer = np.load(f'{path_data_dir_density}/masked_{tracking_type}_{circ}_track_density_summer_multi_{param}.npy')
...
#             vmin = 0
    ...
    if save:
        if not os.path.exists(folder):
            os.makedirs(folder)
        plt.savefig(f"{folder}/masked_{circ}_multi_{season}_sq_{param_sq}.png", dpi=150, bbox_inches="tight", transparent=True)
    plt.close()
